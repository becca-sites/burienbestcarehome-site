...
const QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    options: [
      { value: 'safety', label: "Safety, I'm worried about falls or accidents" },
...
export default function AssessmentForm() {
  ...
  const buildMessage = (scored: ScoreResult): string => {
    const lines: string[] = [
      ...
      `Q5 Concern: ${labelFor('concern', single.concern ?? '')}`,
      `Q6 ADLs: ${labelsFor('adls', multi.adls ?? []).join(', ') || '(none selected)'}`,
      `Q7 Dementia: ${labelFor('dementia', single.dementia ?? '')}`,
      `Q8 Behaviors: ${labelsFor('behaviors', multi.behaviors ?? []).join(', ') || '(none selected)'}`,
      `Q9 Supervision: ${labelFor('supervision', single.supervision ?? '')}`,
      `Q10 Medical: ${labelsFor('medical', multi.medical ?? []).join(', ') || '(none selected)'}`,
      `Q11 Priorities: ${labelsFor('priorities', multi.priorities ?? []).join(', ') || '(none selected)'}`,
      `Q12 Payment: ${labelFor('payment', single.payment ?? '')}`,
      `Q13 Area: ${labelFor('area', single.area ?? '')}`,
